...
// resizeTestBackend creates a box with the given create-time resources and
// returns the backend plus a helper to read back the agent-box container's
// current Resources from the fake clientset.
func resizeTestBackend(t *testing.T, create box.ResourceLimits) (*Backend, *sandboxfake.Clientset, func() corev1.ResourceRequirements) {
	t.Helper()
	b, _, sc := testBackend()
	ctx := context.Background()
	if _, err := b.Create(ctx, box.BoxSpec{
	...
	read := func() corev1.ResourceRequirements {
		sb := getSandbox(t, sc, "tenant-alice")
		for _, c := range sb.Spec.PodTemplate.Spec.Containers {
			if c.Name == boxContainerName {
				return c.Resources
			}
		}
		t.Fatalf("agent-box container not found")
		return corev1.ResourceRequirements{}
	}
	return b, sc, read
}

// TestResizePreservesExistingRequestWhenLimitChanges is the core #1572 bug: a
// resize that only mentions the CPU limit must not silently re-pin the
// scheduler request to the new limit — the box's existing Burstable
// reservation must survive a ceiling-only resize.
func TestResizePreservesExistingRequestWhenLimitChanges(t *testing.T) {
	b, _, read := resizeTestBackend(t, box.ResourceLimits{CPU: "2", CPURequest: "500m"})
	ctx := context.Background()
	if err := b.Resize(ctx, box.BoxRef{Tenant: "alice"}, box.ResourceLimits{CPU: "4"}); err != nil {
		t.Fatalf("Resize: %v", err)
	}
	res := read()
	if lim := res.Limits[corev1.ResourceCPU]; lim.String() != "4" {
		t.Errorf("cpu limit = %q, want 4", lim.String())
	}
	if req := res.Requests[corev1.ResourceCPU]; req.String() != "500m" {
		t.Errorf("cpu request = %q, want 500m (existing request must survive a limit-only resize)", req.String())
	}
}

// TestResizeExplicitRequestOverridesExisting verifies passing --cpu-request
// on a resize does set a new request, mirroring create-time CPURequest.
func TestResizeExplicitRequestOverridesExisting(t *testing.T) {
	b, _, read := resizeTestBackend(t, box.ResourceLimits{CPU: "2", CPURequest: "500m"})
	ctx := context.Background()
	if err := b.Resize(ctx, box.BoxRef{Tenant: "alice"}, box.ResourceLimits{CPU: "4", CPURequest: "1"}); err != nil {
		t.Fatalf("Resize: %v", err)
	}
	res := read()
	if req := res.Requests[corev1.ResourceCPU]; req.String() != "1" {
		t.Errorf("cpu request = %q, want 1", req.String())
	}
}

// TestResizeRequestOnlyLeavesLimitUnchanged verifies a resize that only
// mentions --cpu-request (no --cpu) can adjust the reservation alone,
// resolving the "no way to change the request after create" half of #1572.
func TestResizeRequestOnlyLeavesLimitUnchanged(t *testing.T) {
	b, _, read := resizeTestBackend(t, box.ResourceLimits{CPU: "2", CPURequest: "500m"})
	ctx := context.Background()
	if err := b.Resize(ctx, box.BoxRef{Tenant: "alice"}, box.ResourceLimits{CPURequest: "1"}); err != nil {
		t.Fatalf("Resize: %v", err)
	}
	res := read()
	if lim := res.Limits[corev1.ResourceCPU]; lim.String() != "2" {
		t.Errorf("cpu limit = %q, want 2 (unchanged)", lim.String())
	}
	if req := res.Requests[corev1.ResourceCPU]; req.String() != "1" {
		t.Errorf("cpu request = %q, want 1", req.String())
	}
...
// TestResizeCapsExistingRequestToNewLowerLimit verifies that when the new
// limit drops below the existing (preserved) request, the request is capped
// to the new limit rather than producing an invalid pod spec.
func TestResizeCapsExistingRequestToNewLowerLimit(t *testing.T) {
	b, _, read := resizeTestBackend(t, box.ResourceLimits{CPU: "4", CPURequest: "2"})
	ctx := context.Background()
	if err := b.Resize(ctx, box.BoxRef{Tenant: "alice"}, box.ResourceLimits{CPU: "1"}); err != nil {
		t.Fatalf("Resize: %v", err)
	}
	res := read()
	req := res.Requests[corev1.ResourceCPU]
	lim := res.Limits[corev1.ResourceCPU]
	if req.Cmp(lim) > 0 {
		t.Errorf("request %q exceeds limit %q; want capped", req.String(), lim.String())
	}
	if req.String() != "1" {
		t.Errorf("cpu request = %q, want capped to 1", req.String())
	}
}

// TestResizeMemoryOnlyPreservesCPU verifies a resize touching only memory
// does not wipe out the CPU limits/requests the box already had — the old
// code replaced the whole ResourceRequirements object with only the fields
// the caller mentioned.
func TestResizeMemoryOnlyPreservesCPU(t *testing.T) {
	b, _, read := resizeTestBackend(t, box.ResourceLimits{CPU: "2", CPURequest: "500m", Memory: "1Gi"})
	ctx := context.Background()
	if err := b.Resize(ctx, box.BoxRef{Tenant: "alice"}, box.ResourceLimits{Memory: "2Gi"}); err != nil {
		t.Fatalf("Resize: %v", err)
	}
	res := read()
	if lim := res.Limits[corev1.ResourceCPU]; lim.String() != "2" {
		t.Errorf("cpu limit = %q, want 2 (unchanged by a memory-only resize)", lim.String())
	}
	if req := res.Requests[corev1.ResourceCPU]; req.String() != "500m" {
		t.Errorf("cpu request = %q, want 500m (unchanged by a memory-only resize)", req.String())
	}
	if lim := res.Limits[corev1.ResourceMemory]; lim.String() != "2Gi" {
		t.Errorf("memory limit = %q, want 2Gi", lim.String())
	}
}

// TestResizeNoFieldsIsNoOp verifies Resize with an empty ResourceLimits does
// not touch the Sandbox at all.
func TestResizeNoFieldsIsNoOp(t *testing.T) {
	b, sc, _ := resizeTestBackend(t, box.ResourceLimits{CPU: "2"})
	before := getSandbox(t, sc, "tenant-alice").ResourceVersion
	if err := b.Resize(context.Background(), box.BoxRef{Tenant: "alice"}, box.ResourceLimits{}); err != nil {
		t.Fatalf("Resize: %v", err)
	}
	after := getSandbox(t, sc, "tenant-alice").ResourceVersion
	if before != after {
		t.Errorf("Sandbox was updated by a no-op Resize (resourceVersion %s -> %s)", before, after)
	}
}
