...
// mergeResizeRequirements applies a partial resize onto a container's
// existing ResourceRequirements (#1572). Unlike resourceRequirements (used at
// create, where an unset request defaults to request==limit), a resize that
// doesn't mention a request must leave the box's existing scheduler
// reservation alone — it is not re-derived from a changed limit. Any resource
// (CPU/memory) or field (limit/request) the caller left empty is carried
// forward unchanged from cur, except that an existing request above a newly
// lowered limit is capped down to the new limit (a request may not exceed its
// limit). GPU and any other resource keys in cur are untouched.
func mergeResizeRequirements(cur corev1.ResourceRequirements, r box.ResourceLimits) corev1.ResourceRequirements {
	limits := cur.Limits.DeepCopy()
	requests := cur.Requests.DeepCopy()
	if limits == nil {
		limits = corev1.ResourceList{}
	}
	if requests == nil {
	...
	apply := func(name corev1.ResourceName, limitStr, requestStr string) {
		if limitStr == "" && requestStr == "" {
			return
		}
		limQ, haveLimit := limits[name]
		if limitStr != "" {
			if q, err := resource.ParseQuantity(limitStr); err == nil {
				limQ = q
				haveLimit = true
				limits[name] = q
			}
		...
		if requestStr != "" {
			if q, err := resource.ParseQuantity(requestStr); err == nil && (!haveLimit || q.Cmp(limQ) <= 0) {
				requests[name] = q
			}
			return
		}
		// No explicit request: carry the existing one forward, capped to a
		// newly lowered limit so the pod spec stays valid.
		if existing, ok := requests[name]; ok && haveLimit && existing.Cmp(limQ) > 0 {
			requests[name] = limQ
		}
	}
