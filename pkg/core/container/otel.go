...
// (KEY=value lines, sorted for determinism). Pure function so it's
// trivially testable. The output is consumable by docker-compose's
// `env_file:` directive and by `set -a; . /etc/containarium/otel.env`.
//
...
func RenderOTelEnvFile(env map[string]string) []byte {
	...
	b.WriteString("# Generated by Containarium (monitoring=true). Do not edit.\n")
	b.WriteString("# docker-compose apps: reference via `env_file: " + OTelEnvFilePath + "`.\n")
	for _, k := range keys {
		b.WriteString(k)
		b.WriteByte('=')
		b.WriteString(env[k])
