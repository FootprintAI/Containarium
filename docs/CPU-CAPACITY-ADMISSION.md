...
physical-cores ratio — instead of the arbitrary first-healthy peer picked today.

```
containarium daemon --placement-cpu-aware
```

- **No per-create cost.** Each peer's committed and physical cores are cached on
