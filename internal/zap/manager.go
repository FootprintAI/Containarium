...
func (m *Manager) finalizeScanRun(ctx context.Context, scanRunID string) {
	...
	// Collect fingerprints seen in this scan run. #1398: a failure here
	// must stop the finalize outright, not continue with a nil slice —
	// MarkResolved's own empty-list guard now makes that path a no-op
	// rather than a mass resolution, but a scan run whose actual result
	// is unknown should not be marked "completed" either.
