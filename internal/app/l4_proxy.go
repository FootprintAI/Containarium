...
// EnableL4ProxyProtocol restructures the L4 server to consume a PROXY v2
// header at the front of every connection (when present from a trusted
// sender) so SNI matching can run on the underlying TLS bytes, and the
// catchall route re-emits a PROXY v2 header to its upstream (srv0) so the
// HTTP server's listener_wrapper can recover the real client IP.
//
// The verified pattern (caddy-l4 0.x, sandbox-tested all 4 scenarios):
//
//	{
//	  "routes": [{
//	    "handle": [
//	      {"handler": "proxy_protocol", "allow": ["<senderIP>/32"], "timeout": "5s"},
//	      {"handler": "subroute", "routes": [
//	        // existing SNI routes (unchanged — caddy-l4 byte-pipes raw TLS)
//	        // catchall: same as before but with "proxy_protocol": "v2" added
//	      ]}
//	    ]
//	  }]
//	}
//
// The proxy_protocol handler is lenient on connections without a PROXY
// header (passes through unchanged), so this same config also handles the
// deploy-gap state where the sentinel hasn't been flipped yet — no
// fallback route needed.
//
// trustedSenderCIDRs MUST NOT be empty or wildcard. For prod this is just
// the sentinel's VPC IP (e.g. ["10.130.0.13/32"]).
//
// Idempotent: detects "already wrapped" by checking the first route's
// first handler for "proxy_protocol".
func (m *L4ProxyManager) EnableL4ProxyProtocol(trustedSenderCIDRs []string) error {
	if len(trustedSenderCIDRs) == 0 {
		return fmt.Errorf("EnableL4ProxyProtocol: trustedSenderCIDRs must not be empty")
	}
	...
	// Idempotency: check if first route's first handler is already proxy_protocol.
	if first, ok := routes[0].(map[string]interface{}); ok {
		if hs, ok := first["handle"].([]interface{}); ok && len(hs) > 0 {
			if h0, ok := hs[0].(map[string]interface{}); ok && h0["handler"] == "proxy_protocol" {
	...
	// Tag only the catchall (route with no `match` clause) with
	// proxy_protocol: "v2" so caddy-l4 emits a PROXY header to srv0.
	// SNI passthrough routes leave their proxy handler untouched —
	// gRPC backends typically don't speak PROXY, and caddy-l4 byte-pipes
	// the raw TLS to them which is what they expect.
	for _, r := range routes {
		route, ok := r.(map[string]interface{})
		if !ok {
			continue
		}
		// only the catchall route has no match clause
		if _, hasMatch := route["match"]; hasMatch {
			continue
		}
		hs, _ := route["handle"].([]interface{})
		for _, h := range hs {
	...
	// Wrap all existing routes in a single outer route: proxy_protocol
	// handler (consumes PROXY) + subroute (does the SNI matching on
	// post-strip bytes).
	srv["routes"] = []CaddyL4WrappedOuterRoute{{
		Handle: []interface{}{
			CaddyL4ProxyProtocolHandler{
				Handler: "proxy_protocol",
				Allow:   trustedSenderCIDRs,
				Timeout: "5s",
			},
			CaddyL4SubrouteHandler{
				Handler: "subroute",
				Routes:  routes,
			},
		},
	...
	log.Printf("[L4ProxyManager] PROXY protocol enabled: %d existing routes wrapped behind proxy_protocol handler, allow=%v", len(routes), trustedSenderCIDRs)
	return nil
}
